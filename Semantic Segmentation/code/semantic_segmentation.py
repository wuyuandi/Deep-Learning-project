...
#https://my.oschina.net/liusicong/blog/1606762
#http://warmspringwinds.github.io/tensorflow/tf-slim/2016/12/18/image-segmentation-with-tensorflow-using-cnns-and-conditional-random-fields/?spm=5176.11156381.0.0.34396227yjqHyf
def UnPooling(x):
...
def SemSeg(input_tensor, is_training):
    ...
    #(None, 196, 196, 3)
    #layer1
    conv1 = tf.layers.conv2d(input_tensor,64, kernel_size = (ksiz,ksiz), padding = "valid", activation = tf.nn.relu)
    print("conv1",conv1.shape)
    #layer2 pool
    conv1 = tf.layers.max_pooling2d(inputs = conv1, pool_size = [3, 3], strides = 3)
    
    #layer3
    conv2 = tf.layers.conv2d(conv1, 128, kernel_size = (ksiz, ksiz), padding = "valid", activation = tf.nn.relu)
    
    #layer4 pool
    conv2 = tf.layers.max_pooling2d(inputs = conv2, pool_size = [3, 3], strides = 3)
 
    #layer 5
    conv3 = tf.layers.conv2d(inputs = conv2, filters = 256, kernel_size = [7, 7], padding = "valid", activation = tf.nn.relu)
    print("conv3",conv3.shape)
    #layer 6
    conv4 = tf.layers.conv2d(inputs = conv3, filters = 512, kernel_size = [1, 1], padding = "valid", activation = tf.nn.relu)
    print("conv4",conv4.shape)
    
    #deconv1
    upsampling1 = tf.layers.conv2d_transpose(inputs = conv4, filters = 128, kernel_size = [7, 7], strides = 1, padding = "valid", activation = tf.nn.relu)
    print("upsampling1",upsampling1.shape)

    upsampling1 = tf.image.resize_bilinear(upsampling1,[62,62])
    
    #deconv2
    upsampling2 = tf.layers.conv2d_transpose(inputs = upsampling1, filters = 64, kernel_size = [3, 3], strides = 1, padding = "valid", activation = tf.nn.relu)
    print("upsampling2",upsampling2.shape)
    
    #unpool2
    unpool2 = UnPooling(upsampling2)
    unpool2 = tf.image.resize_bilinear(unpool2,[194,194])
    upsampling3 = tf.layers.conv2d_transpose(inputs = unpool2, filters = 5, kernel_size = [3, 3], strides = 1, padding = "valid", activation = tf.nn.relu)
    
    logits = tf.nn.softmax(upsampling3)
